...
$mail->Subject = 'Сообщение с сайта компании "Dream"';
$mail->Body    = 'Почта пользователя: ' . $email . '<br>' . 'Имя пользователя: ' . $name . '<br>' . 'Сообщение: ' . $msg . '<br>';
$mail->AltBody = '';
...
if(empty($email) || empty($name) || empty($msg)){
    ...
    if(!$mail->send()) {
        header('location: notthank-you.html');
    } else {
